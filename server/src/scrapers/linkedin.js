...
export class LinkedInScraper {
  constructor() {
    ...
    this.searchUrl = 'https://www.linkedin.com/jobs/search';
  }

  async scrapeJobs(searchTerms = ['software engineer', 'developer', 'data scientist'], maxPages = 2) {
    ...
    try {
      for (const searchTerm of searchTerms) {
        ...
        for (let page = 0; page < maxPages; page++) {
          ...
          // Longer delay for LinkedIn to avoid detection
  ...
  async scrapePage(browser, searchTerm, pageNumber) {
    ...
    try {
      ...
      const url = `${this.searchUrl}?keywords=${encodeURIComponent(searchTerm)}&start=${pageNumber * 25}`;
      ...
      const jobs = await page.evaluate(() => {
        ...
        jobElements.forEach((element, index) => {
          try {
            const titleElement = element.querySelector('.job-card-list__title a');
            ...
            const salaryElement = element.querySelector('.job-card-container__salary-info');
            const descriptionElement = element.querySelector('.job-card-container__description');
            const postedElement = element.querySelector('.job-card-container__metadata-item--last');
            ...
            if (titleElement && companyElement && locationElement) {
              ...
              const externalId = jobIdMatch ? jobIdMatch[1] : `linkedin_${Date.now()}_${index}`;
      ...
      logger.error(`Error scraping LinkedIn page ${pageNumber}:`, error);
      return [];
    } finally {
      await page.close();
    }
  ...
  async saveJobsToDatabase(jobs) {
    ...
    for (const job of jobs) {
      try {
        const jobId = `linkedin_${job.sources[0].externalId}_${Date.now()}`;
        ...
        logger.error('Error saving LinkedIn job to database:', error);
      }
    }
  }
}
